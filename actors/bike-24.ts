...
enum MODE {
  TEST = `TEST`, // @title: TEST mode (scrapes only few categories)
  FULL = `FULL`,
}
...
router.addHandler(LABELS.PRODUCTS, async ({ crawler, $, request }) => {
  if (!request.url.includes(`page=`)) { // on first page
    ...
    // FIXME:
    for (let i = 2; i <= Math.min(totalPages, 3); i++) { // skip first page, that is already handled
  ...
  $products.each((i, el) => {
    const pid = $(el).find(`.product-tile__anchor`).attr(`href`).replace(/\D/g, ``) // e.g. `p2421335.html` -> `2421335
    ...
    const product: Output = {
      ...
      originalPrice: prices.oldPrice ? prices.oldPrice * TAX_RATE : prices.price * TAX_RATE,
      currency: `EUR`,
    }
...
void Actor.main(async () => {
  ...
  const proxyConfiguration = inputProxyConfiguration ? await Actor.createProxyConfiguration(inputProxyConfiguration) : undefined
...
async function unblock (session: Session, proxyConfiguration) {
  const browser = await playwright.firefox.launch({
    headless: true, // NOTE: uncomment for debugging
    // TODO: Better pattern to handle both proxy and no proxy
    proxy: proxyConfiguration ? { server: await proxyConfiguration.newUrl(session.id) } : undefined,
  ...
  console.log(`[SESSION] Cloudflare cookie "__cf_bm": ${cfCookie ?? `😱😱😱 not found`}`)
