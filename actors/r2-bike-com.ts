 ...
 * Uses JTL-Shop (PHP) https://www.jtl-software.de/online-shopsystem
 * */

import { Actor } from "apify3"
import { CheerioCrawler, createCheerioRouter, utils as crawleeUtils } from "crawlee"
...
router.addHandler(LABELS.PRODUCTS, async ({ crawler, $, request, log }) => {
  ...
  if (!request.url.match(/_s(\d+)$/)) { // on first page
    const paginationText = $(`.list-pageinfo .page-current`).text().trim() // eg. `Page 1 of 11`
    const match = paginationText.match(/(\d+) of (\d+)/)
    if (!match) log.error(`[PRODUCTS] Failed to parse pagination text: ${paginationText}`)
    const [, currentPage, totalPages] = match ?? []
    if (Number(totalPages) > 1) log.info(`[PRODUCTS] Found ${totalPages} pages, enqueuing`)
    for (let i = 2; i <= Number(totalPages); i++) { // skip first page, that is already handled
  ...
  const products = []
  const $products = $(`#product-list .product-wrapper[itemprop="itemListElement"]`) // itemprop to avoid selecting last fake tile, which is actually "Next page" link
  log.info(`[PRODUCTS] ${request.url} - found ${$products.length} products`)
  $products.each(async (i, el) => {
    const pid = $(`.product-cell`, el).attr(`id`) // result-wrapper_buy_form_106016
      ?.replace(`result-wrapper_buy_form_`, ``) // 106016
    if (!pid) return log.error(`[PRODUCTS] Failed to parse pid from ${i + 1}th product on ${request.url}`)
  ...
  const crawler = new CheerioCrawler({
    ...
    preNavigationHooks: [
      async ({ session }, gotOptions) => {
        ...
        gotOptions.headers.Cookie = userData.cookies.map((c) => `${c.name}=${c.value}`).join(`; `) // real cookies obtained from Firefox
    ...
    sessionPoolOptions: {
      ...
      createSessionFunction: async (sessionPool) => {
        console.log(`[SESSION] Creating new session, will use Firefox to unblock (should take ~10s)`)
...
async function unblock (session: Session) {
